...
import { faWandMagicSparkles, faCircleCheck, faCircleXmark, faLightbulb, faArrowRight, faRotateRight, faHistory } from '@fortawesome/free-solid-svg-icons'
import api from '../utils/api'
import { useSnackbar } from '../App'

function genQueries(domain, brand, keywords) {
  const kw1 = keywords[0] || brand
  const kw2 = keywords[1] || 'services'
  return [
    `${brand} review`,
    `best ${kw1}`,
    `${kw2} ${brand}`,
...
export default function AIVisibility() {
  ...
  const engines = [
    ...
    { key: 'claude',  label: 'Claude',  bg: '#D85A30', color: '#fff', initial: 'C', score: null, soon: false, pending: true },
  ...
  return (
    <div style={{ padding: '1.5rem 2rem', maxWidth: 860 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0,1fr))', gap: 12, marginBottom: 24 }}>
        {engines.map(e => (
          <div key={e.key} style={{
            background: '#fff', border: `1px solid ${e.active && score != null ? SCORE_COLOR(score) + '44' : '#E5E7EB'}`,
            borderRadius: 12, padding: 16, opacity: e.soon ? 0.45 : 1,
          }}>
            ...
            {e.soon ? (
              ...
              <>
                ...
                <div style={{ background: '#F3F4F6', borderRadius: 3, height: 4, overflow: 'hidden', margin: '8px 0 6px' }}>
                  <div style={{ width: score + '%', height: '100%', background: SCORE_COLOR(score), borderRadius: 3, transition: 'width 0.6s ease' }} />
                </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 12, padding: 20, marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 12, color: '#6B7280', marginBottom: 14 }}>
          These are auto-generated based on <strong>{domain}</strong> keywords. Edit them if needed — be specific for best results.
        </div>

        <div style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8, padding: '9px 12px', marginBottom: 14, display: 'flex', gap: 8, alignItems: 'flex-start' }}>
          <FontAwesomeIcon icon={faLightbulb} style={{ color: '#D97706', marginTop: 1, flexShrink: 0 }} />
          <span style={{ fontSize: 12, color: '#92400E', lineHeight: 1.5 }}>
            Include your brand name or city for best results. Generic queries like "web design" will never return your specific site.
          </span>
        </div>
      ...
      {results && (
        <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 12, padding: 20, marginBottom: 16 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: '#111827', marginBottom: 14 }}>
            Results — {cited}/{total} queries cited {domain && `(${domain})`}
          </div>
            ...
            {results.map((r, i) => (
              <div key={i} style={{ border: `1px solid ${r.cited ? '#BBF7D0' : '#FECACA'}`, borderRadius: 10, padding: 14, borderLeft: `4px solid ${r.cited ? '#16A34A' : '#DC2626'}` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: r.excerpt ? 10 : 0 }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 6, background: r.cited ? '#DCFCE7' : '#FEE2E2', color: r.cited ? '#16A34A' : '#DC2626' }}>
      ...
      <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 12, padding: 20, marginBottom: 16 }}>
        ...
        {[
          { icon: faArrowRight, title: 'Submit sitemap to Bing Webmaster Tools', desc: 'ChatGPT uses Bing. Not indexed on Bing = invisible to ChatGPT. Takes 10 mins at webmaster.bing.com.', priority: 'High' },
          { icon: faArrowRight, title: 'Get listed on Trustpilot or G2', desc: 'AI engines use review platforms as trust signals. A free Trustpilot listing is enough to start.', priority: 'High' },
          { icon: faArrowRight, title: 'Add author schema to content pages', desc: 'Named authors with credentials make content more citable by AI engines.', priority: 'Medium' },
          { icon: faArrowRight, title: 'Build Reddit presence', desc: 'Perplexity heavily cites Reddit. Comment in relevant subreddits before posting.', priority: 'Medium' },
        ].map((tip, i) => (
