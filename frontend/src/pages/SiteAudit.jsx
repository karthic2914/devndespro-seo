...
function groupByCategory(checks = []) {
  ...
  const orderedNames = [
    ...CAT_ORDER,
    ...Object.keys(map).filter((n) => !CAT_ORDER.includes(n)),
  ]
  ...
  return orderedNames
    .map((name) => {
      ...
      const score = Math.round(
        issues.reduce(
          (s, i) => s + (i.status === 'pass' ? 100 : i.status === 'warning' ? 55 : 15),
          0
        ) / issues.length
      )
      ...
      {tabs.map((tab) => (
        ...
        >
          {tabIcon[tab.id] && (
            <FontAwesomeIcon
              icon={tabIcon[tab.id]}
              style={{ marginRight: 6, color: tabIconColor[tab.id] }}
            />
          )}
          ...
          {tab.count > 0 && (
            <span
              style={{
                marginLeft: 6,
                background: active === tab.id ? '#F3F4F6' : '#E5E7EB',
                borderRadius: 10,
                padding: '1px 6px',
                fontSize: 10,
                fontWeight: 700,
...
export default function SiteAudit() {
  ...
  async function sendSummaryEmail() {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: 'Failed to send email: ' + (e?.response?.data?.error || 'Unknown error'),
        type: 'error',
      })
    }
  ...
  const visibleIssues = useMemo(() => {
    ...
    return allIssues.filter(
      (i) => (i.category || 'On-Page SEO').toLowerCase().replace(/\s+/g, '_') === activeTab
    )
  }, [allIssues, activeTab])
      ...
      <div ref={captureRef} style={{ padding: '1.5rem 2rem' }}>
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            ...
            {!!shareMsg && (
              <div style={{ width: '100%', textAlign: 'right', fontSize: 11, color: '#6B7280' }}>
                {shareMsg}
              </div>
            )}
          </div>
        ...
        {crawl && (
          <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html:
                      `<div style='text-align:center;margin-bottom:16px;'><img src='/images/devndespro_seo.png' alt='Devndespro SEO' style='height:40px;'/></div>` +
                      emailMessage,
                  }}
                />
            ...
            >
                ...
                >
                  ...
                  <div
                    style={{
                      fontSize: 14,
                      fontWeight: 700,
                      color: crawl.robots?.valid ? '#16A34A' : '#B45309',
                    }}
                  >
              ...
              {!crawl.robots?.valid &&
                Array.isArray(crawl.robots?.issues) &&
                crawl.robots.issues.length > 0 && (
                  <div
                    style={{
                      marginTop: 10,
                      fontSize: 12,
                  ...
                  >
                    ...
                    {Number(crawl.robots.issues[0].line) > 0
                      ? ` (line ${crawl.robots.issues[0].line})`
                      : ''}
                  </div>
                )}
            </div>
          ...
          {visibleIssues.length === 0 ? (
            ...
            visibleIssues.map((issue) => (
              <AuditIssueRow
                ...
                onToggle={() =>
                  setExpandedIdx(expandedIdx === issue._idx ? null : issue._idx)
                }
              />
            ))
          )}
