...
export default function Users() {
  ...
  return <div className="app-shell"><AppSidebar /><div className="app-main"><main className="page-content fade-in users-page">
    ...
    <Card className="users-access-card">
      ...
      {accountsLoading ? <div className="users-empty">Loading accountsâ€¦</div> : accounts.length === 0 ? <div className="users-empty">No accounts yet.</div> : <>
    ...
    <Card padding="0" className="users-invited-card"><div className="users-list-title"><strong>Invited users</strong><span>{users.filter(user => user.status!=='revoked').length}</span></div>{loading ? <div className="users-empty">Loadingâ€¦</div> : users.length===0 ? <div className="users-empty">No users invited yet.</div> : <>
      <div className="users-desktop-only users-invited-table"><div className="users-invited-head"><span>Email</span><span>Project</span><span>Status</span><span>Invited</span><span>Actions</span></div>{users.map(user => <div className="users-invited-row" key={user.id}><strong>{user.email}</strong><span>{user.site_name || 'â€”'}</span><Badge variant={STATUS_VARIANT[user.status] || 'default'}>{user.status==='granted'||user.status==='accepted'?'Active':user.status}</Badge><span>{user.invited_at ? new Date(user.invited_at).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'}) : 'â€”'}</span><div>{(user.status==='pending'||user.status==='granted') && <button type="button" onClick={() => resend(user.id,user.email)} aria-label="Resend invitation"><FontAwesomeIcon icon={faRotateRight} /></button>}{user.status!=='revoked' && <button type="button" className="is-danger" onClick={() => revoke(user.id,user.email)} aria-label="Remove user"><FontAwesomeIcon icon={faTrash} /></button>}</div></div>)}</div>
