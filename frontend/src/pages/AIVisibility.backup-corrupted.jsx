...
export default function AIVisibility() {
  ...
  const chatgptScore = engineScores.chatgpt ?? score ?? 0
  const claudeScore = engineScores.claude ?? claudeResults?.score ?? 0
  const testedEngineCount = [engineScores.chatgpt, engineScores.claude].filter(v => v !== null && v !== undefined).length
  const overallAiScore = testedEngineCount > 0 ? Math.round(([engineScores.chatgpt, engineScores.claude].filter(v => v !== null && v !== undefined).reduce((a, b) => a + b, 0)) / testedEngineCount) : 0
  const totalQueriesTested = history.reduce((sum, h) => sum + ((h.results || []).length || 0), 0)
  const totalCitedQueries = history.reduce((sum, h) => sum + ((h.results || []).filter(r => r.cited).length || 0), 0)
  const coveragePercent = totalQueriesTested > 0 ? Math.round((totalCitedQueries / totalQueriesTested) * 100) : 0
  const lastRun = history.length > 0 ? new Date(history[0].created_at).toLocaleString('en-GB', { day:'2-digit', month:'short', hour:'2-digit', minute:'2-digit' }) : 'No runs yet'
  const estimatedCost = ((history.length || 0) * 0.01).toFixed(2)
...
function getLatestEngineScore(history, engineName) {
  ...
  return (
    <div ref={reportRef} style={{ padding: 'clamp(1rem, 4vw, 1.5rem) clamp(0.75rem, 4vw, 2rem)', maxWidth: 1200, width: '100%', boxSizing: 'border-box' }}>
      <style>{'.ai-vis-dashboard-grid { display:grid; grid-template-columns: 1fr 320px; gap:16px; align-items:start; } @media (max-width: 1024px) { .ai-vis-dashboard-grid { grid-template-columns: 1fr; } } .ai-vis-side-card { background:#fff; border:1px solid #E5E7EB; border-radius:12px; padding:16px; margin-bottom:12px; } .ai-vis-side-title { font-size:13px; font-weight:700; color:#111827; margin-bottom:10px; } .ai-vis-side-row { display:flex; justify-content:space-between; gap:12px; font-size:12px; padding:7px 0; border-bottom:1px solid #F3F4F6; } .ai-vis-side-row:last-child { border-bottom:none; } .ai-vis-side-label { color:#6B7280; } .ai-vis-side-value { color:#111827; font-weight:700; } .ai-vis-trend-box { height:70px; border-radius:8px; background:linear-gradient(180deg,#FFF7ED,#FFFFFF); border:1px solid #FED7AA; display:flex; align-items:center; justify-content:center; color:#F97316; font-size:12px; font-weight:700; } .ai-vis-engines-grid { } @media (max-width: 640px) { .ai-vis-engines-grid { grid-template-columns: repeat(2, 1fr) !important; } } @media (max-width: 400px) { .ai-vis-engines-grid { grid-template-columns: repeat(1, 1fr) !important; } }'}</style>
      ...
      <div className="ai-vis-main-grid">
        <div>
