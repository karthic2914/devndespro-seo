...
router.post('/send-summary', auth, async (req, res) => {
  ...
  const { siteId, subject, message, includeFullReport } = req.body
  ...
  if (includeFullReport) {
    const { rows: auditRows } = await pool.query('SELECT results FROM audit_results WHERE site_id=$1 ORDER BY created_at DESC LIMIT 1', [siteId])
    fullReport = auditRows[0]?.results || null
  }
  try {
    await sendSiteReport({ to, subject, message, fullReport })
    res.json({ ok: true })
  } catch (e) {
