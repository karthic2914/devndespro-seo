...
router.post('/:siteId/ai-visibility/analyse', auth, verifySite, async (req, res) => {
  try {
    ...
    const [sR, mR, kR, aR] = await Promise.all([
      pool.query('SELECT name, url FROM sites WHERE id=$1', [req.siteId]),
      pool.query('SELECT dr, health, clicks FROM seo_metrics WHERE site_id=$1 LIMIT 1', [req.siteId]),
...
DR: ${metrics.dr || 0}, Health: ${metrics.health || 0}, Clicks: ${metrics.clicks || 0}
Keywords: ${keywords}
Audit issues:
${issues}
...
[{"title":"...","action":"2-sentence specific action","priority":"High|Medium|Low"}]`
    ...
    let recommendations = []
    try {
      recommendations = JSON.parse(start >= 0 ? text.slice(start, end + 1) : text)
    } catch {
      recommendations = [{ title: 'Run a site audit first', action: 'Complete a site audit to get personalised AI visibility recommendations.', priority: 'High' }]
    }

    res.json({ recommendations, url: site.url, engine })
  } catch (e) {
    console.error('AI visibility analyse error:', e)
